                ...
                <nav class="flex items-center gap-3 text-sm text-stone-300">
                    ...
                    @elseif (auth()->user()?->isAdmin())
                        ...
                        <form action="{{ route('logout') }}" method="POST" data-session-exit-form>
                            @csrf
                            <button type="submit" class="rounded-full border border-white/10 px-4 py-2 transition hover:border-amber-300/50 hover:text-white" data-loading-label="Signing out...">Sign out</button>
                        </form>
                    @endguest
                </nav>
            </div>
